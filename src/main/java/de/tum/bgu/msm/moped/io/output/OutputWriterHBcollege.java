...
public class OutputWriterHBcollege {

    private static final Logger logger = Logger.getLogger(OutputWriterHBcollege.class);
    private final DataSet dataSet;

    public OutputWriterHBcollege(DataSet dataSet) {
    ...
    public void writeOutTG () throws FileNotFoundException  {
        ...
        for (long zoneId : dataSet.getZones().keySet()){
            ...
            for (int hhTypeId : dataSet.getHhTypes().keySet()){
                double pr = dataSet.getHbCollegeProduction().get(zoneId,hhTypeId);
                sumTripGen += pr;
            }
            tripGen.append(',');
    ...
    public void writeOutWM () throws FileNotFoundException  {
        ...
        for (long zoneId : dataSet.getZones().keySet()){
            ...
            for (int hhTypeId : dataSet.getHhTypes().keySet()){
                double walkTrip = dataSet.getHbCollegeWalk().get(zoneId,hhTypeId);
    ...
    public void writeOutTripLength () throws FileNotFoundException  {
        ...
        //write data
        for (long origin : dataSet.getHbCollegeDistribution().rowKeySet()){
            long superPAZ = dataSet.getZone(origin).getSuperPAZId();
            double sumDistribution = 0.0;
            distribution.append(origin);
            for (long destination : dataSet.getHbCollegeDistribution().columnKeySet()){
                double trips = dataSet.getHbCollegeDistribution().get(origin,destination);
                if (trips != 0.0){
        ...
        for (long origin : dataSet.getHbCollegeDistribution().rowKeySet()){
            ...
            for (long destination : dataSet.getHbCollegeDistribution().columnKeySet()){
                ...
                if (superPAZ == destination){
                    distribution.append(",");
                    distribution.append(dataSet.getHbCollegeDistribution().get(origin,destination));
                }
            }
            distribution.append('\n');
        }

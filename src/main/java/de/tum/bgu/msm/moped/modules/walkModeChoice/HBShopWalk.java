...
public class HBShopWalk {

    private static final Logger logger = Logger.getLogger(HBShopWalk.class);
    ...
    public void run () {
        ...
        walkTripsCalculator();
        dataSet.setHbShopWalk(WalkTrip);
    }
    ...
    private void walkUtilityCalculator() {
        for (long zoneId : dataSet.getZones().keySet()) {
            ...
            for (int hhTypeId : dataSet.getHhTypes().keySet()) {
                ...
                double utilityHousehold = -0.145 + -4.377 + 0.191*(hhType.getHouseholdSize()==2?1:0) + -0.242*(hhType.getAge()==3?1:0)+ 0.208*(hhType.getWorkers()==1?1:0)+ 0.301*(hhType.getWorkers()==2?1:0) +

...
public class OutputWriterHBSchool {

    private static final Logger logger = Logger.getLogger(OutputWriterHBSchool.class);
    private final DataSet dataSet;

    public OutputWriterHBSchool(DataSet dataSet) {
    ...
    public void writeOutTG () throws FileNotFoundException  {
        ...
        for (long zoneId : dataSet.getZones().keySet()){
            ...
            for (int hhTypeId : dataSet.getHhTypes().keySet()){
                double pr = dataSet.getHbSchoolProduction().get(zoneId,hhTypeId);
                sumTripGen += pr;
            }
            tripGen.append(',');
    ...
    public void writeOutWM () throws FileNotFoundException  {
        ...
        for (long zoneId : dataSet.getZones().keySet()){
            ...
            for (int hhTypeId : dataSet.getHhTypes().keySet()){
                double walkTrip = dataSet.getHbSchoolWalk().get(zoneId,hhTypeId);
    ...
    public void writeOutTripLength () throws FileNotFoundException  {
        ...
        //write data
        for (long origin : dataSet.getHbSchoolDistribution().rowKeySet()){
            long superPAZ = dataSet.getZone(origin).getSuperPAZId();
            double sumDistribution = 0.0;
            distribution.append(origin);
            for (long destination : dataSet.getHbSchoolDistribution().columnKeySet()){
                double trips = dataSet.getHbSchoolDistribution().get(origin,destination);
                if (trips != 0.0){
        ...
        for (long origin : dataSet.getHbSchoolDistribution().rowKeySet()){
            ...
            for (long destination : dataSet.getHbSchoolDistribution().columnKeySet()){
                ...
                if (superPAZ == destination){
                    distribution.append(",");
                    distribution.append(dataSet.getHbSchoolDistribution().get(origin,destination));
                }
            }
            distribution.append('\n');
        }

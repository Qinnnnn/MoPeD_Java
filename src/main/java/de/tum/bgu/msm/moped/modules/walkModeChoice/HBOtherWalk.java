...
public class HBOtherWalk {

    private static final Logger logger = Logger.getLogger(HBOtherWalk.class);
    ...
    public void run () {
        ...
        walkTripsCalculator();
        dataSet.setHbOtherWalk(WalkTrip);
    }
    ...
    private void walkUtilityCalculator() {
        for (long zoneId : dataSet.getZones().keySet()) {
            ...
            for (int hhTypeId : dataSet.getHhTypes().keySet()) {
                ...
                double utilityHousehold = -4.377 + 0.191*(hhType.getHouseholdSize()==2?1:0) + -0.242*(hhType.getAge()==3?1:0)+ 0.208*(hhType.getWorkers()==1?1:0)+ 0.301*(hhType.getWorkers()==2?1:0) +
    ...
    private void walkTripsCalculator() {
        for (long zoneId : dataSet.getZones().keySet()) {
            ...
            for (int hhTypeId : dataSet.getHhTypes().keySet()) {
                ...
                double walkTrips = walkProbability * dataSet.getHbOtherTripGen().get(zoneId,hhTypeId);
                WalkTrip.put(zoneId,hhTypeId,walkTrips);
                totalWalkTrips += walkTrips;
            }
            dataSet.getZone(zoneId).setHbOtherWalkTrips(totalWalkTrips);
        }
    }
}

...
import static de.tum.bgu.msm.moped.data.Purpose.NHBO;
import static de.tum.bgu.msm.moped.data.Purpose.NHBW;
...
public final class NonHomeBasedDistribution extends RandomizableConcurrentFunction<Void> {
    ...
    public NonHomeBasedDistribution(DataSet dataSet, List<MopedZone> zoneList, MultiNodePathCalculator pathCalculator,
    ...
    public Void call() {
        ...
        for (MopedZone origin : zoneList) {
            ...
            for ( int destinationId : utilityMatrices.get(NHBW).keys().elements()) {
                SuperPAZ destination = dataSet.getSuperPAZ(destinationId);  //generate toNodes vector (toPAZ) and calculate Least Cost Path to all destination nodes
                EnumMap<Purpose, OpenIntFloatHashMap> utilityListPAZ = new EnumMap<>(Purpose.class);

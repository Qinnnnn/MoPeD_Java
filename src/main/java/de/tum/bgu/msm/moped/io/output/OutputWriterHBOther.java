...
public class OutputWriterHBOther {

    private static final Logger logger = Logger.getLogger(OutputWriterHBOther.class);
    private final DataSet dataSet;

    public OutputWriterHBOther(DataSet dataSet) {
    ...
    public void writeOutTG () throws FileNotFoundException  {
        ...
        for (long zoneId : dataSet.getZones().keySet()){
            ...
            for (int hhTypeId : dataSet.getHhTypes().keySet()){
                double pr = dataSet.getHbOtherTripGen().get(zoneId,hhTypeId);
                sumTripGen += pr;
            }
            tripGen.append(',');
    ...
    public void writeOutWM () throws FileNotFoundException  {
        ...
        for (long zoneId : dataSet.getZones().keySet()){
            ...
            for (int hhTypeId : dataSet.getHhTypes().keySet()){
                double walkTrip = dataSet.getHbOtherWalk().get(zoneId,hhTypeId);
    ...
    public void writeOutTripLength () throws FileNotFoundException  {
        ...
        //write data
        for (long origin : dataSet.getHbOtherDistribution().rowKeySet()){
            long superPAZ = dataSet.getZone(origin).getSuperPAZId();
            double sumDistribution = 0.0;
            distribution.append(origin);
            for (long destination : dataSet.getHbOtherDistribution().columnKeySet()){
                double trips = dataSet.getHbOtherDistribution().get(origin,destination);
                if (trips != 0.0){
        ...
        for (long origin : dataSet.getHbOtherDistribution().rowKeySet()){
            ...
            for (long destination : dataSet.getHbOtherDistribution().columnKeySet()){
                ...
                if (superPAZ == destination){
                    distribution.append(",");
                    distribution.append(dataSet.getHbOtherDistribution().get(origin,destination));
                }
            }
            distribution.append('\n');
        }

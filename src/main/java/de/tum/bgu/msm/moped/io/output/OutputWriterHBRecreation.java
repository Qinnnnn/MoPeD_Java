...
public class OutputWriterHBRecreation {

    private static final Logger logger = Logger.getLogger(OutputWriterHBRecreation.class);
    private final DataSet dataSet;

    public OutputWriterHBRecreation(DataSet dataSet) {
    ...
    public void writeOutTG () throws FileNotFoundException  {
        ...
        for (long zoneId : dataSet.getZones().keySet()){
            ...
            for (int hhTypeId : dataSet.getHhTypes().keySet()){
                double pr = dataSet.getHbRecreationTripGen().get(zoneId,hhTypeId);
                sumTripGen += pr;
            }
            tripGen.append(',');
    ...
    public void writeOutWM () throws FileNotFoundException  {
        ...
        for (long zoneId : dataSet.getZones().keySet()){
            ...
            for (int hhTypeId : dataSet.getHhTypes().keySet()){
                double walkTrip = dataSet.getHbRecreationWalk().get(zoneId,hhTypeId);
    ...
    public void writeOutTripLength () throws FileNotFoundException  {
        ...
        //write data
        for (long origin : dataSet.getHbRecreationDistribution().rowKeySet()){
            long superPAZ = dataSet.getZone(origin).getSuperPAZId();
            double sumDistribution = 0.0;
            distribution.append(origin);
            for (long destination : dataSet.getHbRecreationDistribution().columnKeySet()){
                double trips = dataSet.getHbRecreationDistribution().get(origin,destination);
                if (trips != 0.0){
        ...
        for (long origin : dataSet.getHbRecreationDistribution().rowKeySet()){
            ...
            for (long destination : dataSet.getHbRecreationDistribution().columnKeySet()){
                ...
                if (superPAZ == destination){
                    distribution.append(",");
                    distribution.append(dataSet.getHbRecreationDistribution().get(origin,destination));
                }
            }
            distribution.append('\n');
        }

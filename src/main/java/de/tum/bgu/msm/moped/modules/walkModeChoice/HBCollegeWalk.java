...
public class HBCollegeWalk {
    ...
    public void run () {
        ...
        walkTripsCalculator();
        dataSet.setHbCollegeWalk(WalkTrip);
    }
    ...
    private void walkTripsCalculator() {
        for (long zoneId : dataSet.getZones().keySet()) {
            ...
            for (int hhTypeId : dataSet.getHhTypes().keySet()) {
                ...
                double walkTrips = walkProbability * dataSet.getHbCollegeProduction().get(zoneId,hhTypeId);
                WalkTrip.put(zoneId,hhTypeId,walkTrips);
                totalWalkTrips += walkTrips;
            }
            dataSet.getZone(zoneId).setHbCollegeWalkTrips(totalWalkTrips);
        }
    }
}
